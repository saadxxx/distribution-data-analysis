...
核心结论
1. 配送服务优化  
   • 货品4→西北线路（平均配送时效156小时）  

   • 货品2→马来西亚线路（平均配送时效213小时）

   这两条线路存在显著时效问题，需紧急优化配送流程

2. 市场潜力区域  
   • 货品2在华东地区（平均月销量18,200件）  

   该区域尚有显著市场空间，值得扩大投入  
   • 货品2在西北地区（配送时效156小时+拒收率8.7%）  

   该区域投入产出比失衡，建议减少资源分配

3. 商品质量问题  
   • 货品1、2、4的退货率分别为：7.2%、6.5%、5.9%（高于行业平均3.5%）  
...
![配送时效分布图](outputs/delivery_performance.png)

（*图注：横轴为平均配送天数，纵轴为区域-商品组合，颜色区分各商品线*）

月度销售趋势

![月度销售柱状图](outputs/monthly_sales.png)

（*图注：不同颜色代表不同商品，显示销量随月份波动情况*）

质量问题分布

![质量问题饼图](outputs/quality_issues.png)
...
> 注：项目数据已作脱敏处理，原始文件存储在data目录，清洗后数据在cleaned_data目录

✏️ 分析说明
• 所有图表均基于企业202X年X-X月的运营数据
